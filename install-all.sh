...
case "$op" in
  ...
  all) if $DRY_RUN; then "$0" --dry-run packages; "$0" --dry-run dotfiles; "$0" --dry-run shell; "$0" --dry-run desktop-launchers; else "$0" packages; "$0" dotfiles; "$0" shell; "$0" desktop-launchers; fi;;
  *) usage; exit 2;;
esac
